...
fn validate<'a>(
    ...
    match cert.signature_algorithm.algorithm {
        ...
        ECDSA_WITH_SHA256_ALGORITHM => match pbk {
            PublicKey::ECDSA(pbk) => validate_ecdsa_sha256(&payload, &cert.signature_value, &pbk),
            _ => Err(ValidationError::UnsupportedPublicKeyAlgorithm),
        },
        ECDSA_WITH_SHA384_ALGORITHM => match pbk {
            PublicKey::ECDSA(pbk) => validate_ecdsa_sha384(&payload, &cert.signature_value, &pbk),
            _ => Err(ValidationError::UnsupportedPublicKeyAlgorithm),
        },
...
fn validate_ecdsa_sha384(
    ...
    match curve {
        ...
        ECDSACurve::CurveP384(affine_point) => {
            ...
            let hashed = &sha2::Sha384::digest(payload);
            let payload = p384::FieldBytes::from_slice(&hashed);
