...
pub fn get_statemint_asset(asset: &MultiAsset) -> Result<(u128, u128), ()>{
    return match asset {
        MultiAsset { fun: Fungible(amount), id: Concrete (
            MultiLocation {
...
pub struct StatemintTransactor <
    ...
    CheckAsset,
    CheckingAccount,
>(PhantomData<(Runtime, Assets, Matcher, AccountIdConverter, AccountId, CheckAsset, CheckingAccount)>);
impl<
...
> TransactAsset
for StatemintTransactor<Runtime, Assets, Matcher, AccountIdConverter, AccountId, CheckAsset, CheckingAccount>
{
    ...
    fn deposit_asset(what: &MultiAsset, who: &MultiLocation) -> XcmResult {
        ...
        >::deposit_asset(what, who)
            ...
            let (asset_id, _amount) = get_statemint_asset(what).map_err(|_| XcmError::AssetNotFound)?;
            let pallet_assets_account: <Runtime as frame_system::Config>::AccountId = <Runtime as crate::Config>::PalletId::get().into_account_truncating();
            let raw_origin= RawOrigin::<<Runtime as frame_system::Config>::AccountId>::Signed(pallet_assets_account.clone());
            let pallet_origin: <Runtime as frame_system::Config>::Origin = raw_origin.into();

            let asset_creation = pallet_assets::Pallet::<Runtime>::create(
                pallet_origin,
                asset_id as u32,
