...
use sp_runtime::{bounded_vec, generic, parameter_types, AccountId32, BoundedVec};
use xcm::prelude::*;

use crate::{
    payments, Advertisement, AttestationChain, Fulfillment, JobAssignmentUpdate,
    JobAssignmentUpdateBarrier, JobRegistration, LockAndPayAsset, PricingVariant,
    RevocationListUpdateBarrier, RewardAssetId, Script, SerialNumber, MAX_PRICING_VARIANTS,
};
    ...
    pub fn build(self) -> sp_io::TestExternalities {
        ...
        pallet_assets::GenesisConfig::<Test> {
            ...
            accounts: vec![
                ...
                (22, processor_account_id(), INITIAL_BALANCE),
            ],
        }
        .assimilate_storage(&mut t)
        .unwrap();
...
impl crate::Config for Test {
    ...
    // type AssetTransactor = NoopTransactor;
...
/// A noop transactor that does nothing on locking/paying assets. Used for testing when statemint not available.
pub struct NoopTransactor;

impl LockAndPayAsset<Test> for NoopTransactor {
    fn lock_asset(
        _asset: MultiAsset,
        _owner: <<Test as frame_system::Config>::Lookup as sp_runtime::traits::StaticLookup>::Source,
    ) -> Result<(), ()> {
...
pub fn advertisement(price_per_cpu_millisecond: u128, capacity: u32) -> Advertisement<AccountId> {
    let pricing: BoundedVec<PricingVariant, ConstU32<MAX_PRICING_VARIANTS>> =
        bounded_vec![PricingVariant {
            reward_asset: RewardAssetId(MultiLocation {
                parents: 1,
...
pub fn job_assignment_update_for(
    ...
    vec![JobAssignmentUpdate {
        operation: crate::JobAssignemntUpdateOperation::Add(0),
...
pub fn asset(value: u128) -> MultiAsset {
    MultiAsset {
        id: Concrete(MultiLocation {
            parents: 1,
            interior: X3(Parachain(1000), PalletInstance(50), GeneralIndex(22)),
