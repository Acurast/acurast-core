...
pub(crate) mod tests {
    ...
    fn should_persist_progress_across_runs() {
        ...
        // version not available in db -> None
        assert_eq!(
            load_decode::<Backend, Option<u32>>(&*backend, VERSION_KEY).unwrap(),
            None
        );
        ...
        run_test_with_mmr_gadget_pre_post_using_client(
            ...
            |client| async move {
                let a1 = client
                    ...
                    .await;
                let a2 = client
                    .import_block(&BlockId::Number(1), b"a2", vec![1])
                    .await;
                let a3 = client
                    .import_block(&BlockId::Number(2), b"a3", vec![2])
                    .await;
        ...
        run_test_with_mmr_gadget_pre_post_using_client(
            ...
            |client| async move {
                let a4 = client
                    .import_block(&BlockId::Number(3), b"a4", vec![3])
                    .await;
                let a5 = client
                    .import_block(&BlockId::Number(4), b"a5", vec![4])
                    .await;
                let a6 = client
                    .import_block(&BlockId::Number(5), b"a6", vec![5])
                    .await;
                ...
                assert_eq!(
                    load_persistent::<Block, Backend>(&*client.backend).unwrap(),
                    Some(6)
                );
            },
        );
