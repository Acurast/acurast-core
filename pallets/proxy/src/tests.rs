...
pub fn relay_ext() -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Runtime> {
		balances: vec![(ALICE, INITIAL_BALANCE), (para_account_id(2000), INITIAL_BALANCE)],
	}
		.assimilate_storage(&mut t)
		.unwrap();
...
mod network_tests {
	...
	fn dmp() {
		...
		let remark =
			acurast_runtime::Call::System(frame_system::Call::<acurast_runtime::Runtime>::remark_with_event {
				remark: vec![1, 2, 3],
			});
		Relay::execute_with(|| {
	...
	fn xcmp() {
		...
		let remark =
			proxy_runtime::Call::System(frame_system::Call::<proxy_runtime::Runtime>::remark_with_event {
				remark: vec![1, 2, 3],
			});
	...
	fn withdraw_and_deposit() {
		...
		Relay::execute_with(|| {
			...
			assert_eq!(relay_chain::Balances::free_balance(para_account_id(2001)), send_amount);
		});
	}
...
mod proxy_calls {
	...
	fn submit_attestation(){
		...
		CumulusParachain::execute_with(|| {
			...
			let x_origin = Origin::signed(hex!("b8bc25a2b4c0386b8892b43e435b71fe11fa50533935f027949caf04bcce4694").into()).into();
