...
// parent re-exports
use emulations::runtimes::{proxy_runtime, acurast_runtime, polkadot_runtime, statemint_runtime};
...
pub fn acurast_ext(para_id: u32) -> sp_io::TestExternalities {
	...
	let PALLET_ASSETS_ACCOUNT: <Runtime as frame_system::Config>::AccountId = <Runtime as pallet_acurast::Config>::PalletId::get().into_account_truncating();

	pallet_balances::GenesisConfig::<Runtime> {
		balances: vec![(ALICE, INITIAL_BALANCE), (PALLET_ASSETS_ACCOUNT, INITIAL_BALANCE), (BOB, INITIAL_BALANCE)],
	}
...
pub fn statemint_ext(para_id: u32) -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Runtime> {
		balances: vec![(BURN_ACCOUNT, INITIAL_BALANCE), (ALICE, INITIAL_BALANCE), (sibling_para_account_id(2000), INITIAL_BALANCE), (sibling_para_account_id(2001), INITIAL_BALANCE)],
	}
		.assimilate_storage(&mut t)
		.unwrap();
...
pub fn polkadot_ext() -> sp_io::TestExternalities {
	...
	pallet_balances::GenesisConfig::<Runtime> {
		...
		balances: vec![(ALICE, INITIAL_BALANCE), (child_para_account_id(2000), INITIAL_BALANCE),
...
type CumulusXcmPallet = pallet_xcm::Pallet<proxy_runtime::Runtime>;
...
mod network_tests {
	...
	fn xcmp() {
		...
		let remark = proxy_runtime::Call::System(frame_system::Call::<proxy_runtime::Runtime>::remark_with_event {
			remark: "Hello from acurast!".as_bytes().to_vec(),
		});
		AcurastParachain::execute_with(|| {
			assert_ok!(acurast_runtime::PolkadotXcm::send_xcm(
	...
	fn reserve_transfer() {
		...
		PolkadotRelay::execute_with(|| {
			assert_ok!(PolkadotXcmPallet::reserve_transfer_assets(
				...
				Box::new( MultiAsset { id: Concrete( MultiLocation { parents: 0, interior: Here} ) ,fun: Fungible(withdraw_amount)}.into() ),
				0,
			));
			assert_eq!(
		...
		AcurastParachain::execute_with(|| {
			let events = acurast_runtime::System::events();
			...
			let pallet_balance = pallet_balances::Pallet::<acurast_runtime::Runtime>::free_balance(&ALICE);
			assert!( pallet_balance < (full_deposit + fee_margin) &&
	...
	fn withdraw_and_deposit() {
		...
		PolkadotRelay::execute_with(|| {
			let acurast_balance = polkadot_runtime::Balances::free_balance(child_para_account_id(2000));
			let proxy_balance = polkadot_runtime::Balances::free_balance(child_para_account_id(2001));
	...
	fn query_holding() {
		...
		AcurastParachain::execute_with(|| {
			use acurast_runtime::{System, Event};
...
mod statemint_backed_native_assets {
	...
	fn reserve_transfer_mint_native() {
		...
		let test = sibling_para_account_id(2000);
...
mod job_payments {
	...
	fn pallet_assets_is_callable_in_runtime() {
		...
		AcurastParachain::execute_with(|| {
			...
			let result = pallet_assets::Pallet::<AcurastRuntime>::create(
				pallet_origin,
				420,
	...
	fn create_job_and_fulfill_local() {
		let job_token = MultiAsset {
			id: Concrete(MultiLocation { parents: 1, interior: X3(Parachain(1000), PalletInstance(50), GeneralIndex(69)) }),
			fun: Fungible(INITIAL_BALANCE / 2)
		};
		...
		AcurastParachain::execute_with(|| {
			...
			let pallet_account: <AcurastRuntime as frame_system::Config>::AccountId = <AcurastRuntime as pallet_acurast::Config>::PalletId::get().into_account_truncating();
			let pallet_balance_69 = AcurastMinter::balance(69, pallet_account);
			let alice_balance_false = AcurastMinter::balance(42, &ALICE);
			let alice_balance_native = acurast_runtime::Balances::free_balance(&ALICE);
			let x = 10;
		});
		...
		AcurastParachain::execute_with(|| {
			...
			let alice_balance_native = acurast_runtime::Balances::free_balance(&ALICE);
			let bob_balance_69 = AcurastMinter::balance(69, &BOB);
			let bob_balance_native = acurast_runtime::Balances::free_balance(&BOB);
			let pallet_account: <AcurastRuntime as frame_system::Config>::AccountId = <AcurastRuntime as pallet_acurast::Config>::PalletId::get().into_account_truncating();
			let pallet_balance_69 = AcurastMinter::balance(69, pallet_account);
